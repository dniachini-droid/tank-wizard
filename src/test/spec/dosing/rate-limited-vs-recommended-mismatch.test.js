/* Regression test for the 13 Aug build-cycle's adjudicated item #10
 * (static-analyst/S2, confirmed, deliberately left unfixed — the fixer plan
 * (.agent/log/2026-08-13-build-1.md, wave 3) deferred it as too
 * architecturally risky for a same-night patch: "touches
 * dose-recommendation logic in all three chemistry engines; real bug but
 * not small/unambiguous — needs a full implementer + domain-verifier
 * round." Re-verified still live on 15 Aug main during the PR #3 rebase.
 *
 ...
 * engines (alkalinity.js:911/917, calcium.js:619/625, helpers.js:1123/1129 —
 * same pattern, byte-identical order). `rateLimitDose` sets `out.rateLimited`
 * (what ErrorBoundary.jsx:161 renders verbatim as the "Held to X mL/day"
 ...
 * DoseChangeSheet, ErrorBoundary.jsx, actually prefills) is set from
 ...
 * file, confirmed via grep, re-confirmed on 15 Aug main).
 *
