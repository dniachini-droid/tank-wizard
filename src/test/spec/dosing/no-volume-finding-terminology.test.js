/* Regression test for the terminology half of adjudicated.md item #6
 * (domain-verifier/S2, confirmed, DUPLICATE-OF-TW-017 — `.agent/backlog.md`,
 * untagged, so not implementer-eligible tonight per AGENTS.md rule 3's
 * untagged-items-read-only rule; this only adds coverage, it does not touch
 * the banned copy itself).
 *
 * docs/spec/surfaces-and-messaging.md §5 terminology registry: the required
 ...
 *
 * THIS TEST IS EXPECTED TO FAIL until TW-017 lands (untagged, needs
 * [approved] before any implementer touches the copy). It exists so the
