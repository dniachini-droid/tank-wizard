...
const STATUS_COLOR = { ok: "#0B7C86", low: "#926A09", high: "#C4285B", unknown: "#9FB0AE" };
...
const STABILITY_COLOR = { green: "#0B7C86", amber: "#A2621B", red: "#C4285B", unknown: "#9FB0AE" };
...
function safetyCapFor(defs, latestByParam) {
...
const isCorrectionState = (state) => CORRECTION_STATES.has(state);
...
function LogResultPopup({ result, onClose, readings = [], onOpenDosing }) {
  useEscape(onClose);
...
const CORRECTION_MAX_RATE = { alkalinity: 0.5, calcium: 20, magnesium: 25 };
...
const SAFE_DAILY_RISE = { ...CORRECTION_MAX_RATE };
...
function applyDoseConstraints(next, out, def, settings, readings, doseLog) {
...
function rateLimitDose(applied, out, def, settings, effect) {
 ...
 * threshold passed in. */
function trendConfirmed(out, readingCount, spanDays, smallTrend) {
...
function noteCurrentAndInterventions(out, all, def, doseLog, correctionPlans) {
...
function solveSlowEffect(key, readings, doseLog, waterChanges, settings, corrections) {
...
const DOSE_STEP_CAP = 0.25;

function capDoseStep(wanted, currentDose, level, def, rate) {
...
const BRACKET_MEMORY_DAYS = 45;

function doseObservations(doseLog, readings, paramKey, effectPerMl, today) {
...
function bracketDose(wanted, currentDose, currentRate, observations, effectPerMl, today) {
...
function pendingCorrection(corrections, def, current, today, settingsForCorrection, readingsForCorrection) {
...
const CORRECTION_PACE = { gentle: 0.25, steady: 0.5, quick: 1.0 };
...
function correctionPlanFor(plans, key) {
...
function correctionProgress(plan, def, readings, today, maintenanceNow) {
  if (!plan) return null;
  const rows = (readings || [])
...
function canLowerByDose(def, dailySupply) {
...
function proposeCorrection(a, def, settings, pace) {
...
function doseDriftedFrom(maintenanceDose, currentDose, key, outOfBand) {
...
function ratePhrase(a, def) {
  ...
  /* Every correction state carries the plan. Three returns did not — two
     ...
     not. */
function doseStatus(a, def, todayIso, settings, latestByParam, doseLog, waterChanges) {
...
function kitNoise(paramKey, settings) {
...
function settleWindow(paramKey, dailySupply, settings) {
...
function deriveTankState(input) {
  const {
...
function CorrectionPanel({ def, state, offers, onStart, onCancel, onFinish }) {
