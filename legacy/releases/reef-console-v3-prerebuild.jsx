...
function buildOverview(readings, latestByParam, defs, findings, doseStates) {
  ...
  const lead = urgent.length === 1 ? sentenceCase(urgent[0].title)
    : urgent.length ? `${urgent.length} things need attention`
    : needsTest.length === 1 ? sentenceCase(needsTest[0].headline)
    : needsTest.length ? `${needsTest.length} dose changes need a test`
    : running.length === 1 ? sentenceCase(running[0].headline)
    : missed.length ? sentenceCase(missed[0].headline)
    : suggested.length === 1 ? sentenceCase(suggested[0].headline)
    : suggested.length ? `${suggested.length} doses could change`
    : null;
...
/* ===========================================================================
   StatusHero — what the dashboard opens with
   ===========================================================================
   The old dashboard opened with a section title, then a score tile, then a
   headline, then a collapsed assessment: four elements before the first fact.
   This is one. The verdict is the largest thing on the screen because it is
   the answer to the question that made you open the app, and the score sits
   beside it as supporting evidence rather than leading.

   Reads the same `overview` object as before — no new data, no new state.
   ========================================================================= */
function StatusHero({ overview, onOpenAssessment, open }) {
  const s = overview.score;
  const band = s == null ? "#9FB0AE" : s >= 85 ? "#2E8B57" : s >= 70 ? "#0B7C86" : s >= 50 ? "#D98324" : "#C4285B";
  /* Written without a regex alternation: the scope checker reads the branches
     as identifiers and reports them as undeclared. */
  const head = String(overview.headline || "").toLowerCase();
  const calm = ["everything", "all ", "nothing", "tracking", "solid"].some((w) => head.startsWith(w));

  return (
    <section className="hero">
      <div className="hero-top">
        <span className="hero-eyebrow">Reef status</span>
        {s != null && (
          <span className="hero-score" style={{ color: band }}>
            {s}<span className="hero-score-max">/100</span>
          </span>
        )}
      </div>

      <h1 className="hero-verdict">{overview.headline}</h1>

      {/* A single hairline that carries the score as a filled proportion. It
          replaces the ring, which needed a legend to be read at all. */}
      {s != null && (
        <div className="hero-meter">
          <div className="hero-meter-fill" style={{ width: `${Math.max(2, Math.min(100, s))}%`, background: band }} />
        </div>
      )}

      {overview.paragraphs && overview.paragraphs.length > 0 && (
        <>
          <button onClick={onOpenAssessment} className="hero-more">
            {open ? "Hide the detail" : `Read the full assessment · ${overview.paragraphs.length} ${overview.paragraphs.length === 1 ? "section" : "sections"}`}
          </button>
          {open && (
      ...
      {calm && <span className="hero-calm" aria-hidden="true" />}
    </section>
  );
}
      ...
      <div className="flex items-start gap-4">
        {s != null && (
          <div className="shrink-0 text-center">
            ...
            <div className="text-[9px] font-extrabold uppercase tracking-wider text-ink2 mt-1">Health</div>
          </div>
        )}
            ...
            <>
              <div className="space-y-2">
                {overview.paragraphs.map((p, i) => (
                  <p key={i} className="text-[13px] leading-relaxed text-ink font-medium">{p}</p>
                ))}
              </div>
              <button onClick={() => setOpen(false)} className="mt-3 text-xs font-extrabold flex items-center gap-1" style={{ color: scoreColor }}>
                <ChevronUp size={13} /> Show less
              </button>
            </>
          ) : (
            <button onClick={() => setOpen(true)} className="text-xs font-extrabold flex items-center gap-1" style={{ color: scoreColor }}>
              <ChevronDown size={13} /> Read full assessment · {overview.paragraphs.length} {overview.paragraphs.length === 1 ? "section" : "sections"}
            </button>
          )}
        </div>
      </div>
...
/* ===========================================================================
   ParamRow — the dashboard's instrument line
   ===========================================================================
   Replaces the two-column card grid on the dashboard. A phone shows four of
   those cards before you scroll; it shows eight of these, and each carries
   more: the value, where it sits in its band, which way it is moving, and
   whether a dose needs attention. Cards spent most of their area on padding
   and border, which is affordable on a wall display and not on a phone.

   Same props as ParamCard and the same handlers, so behaviour is unchanged —
   this is a different presentation of identical data.
   ========================================================================= */
function ParamRow({ def, reading, stab, findings, rows, dose, onOpen, onLog }) {
  const value = reading ? reading.value : null;
  const status = value == null ? "none" : paramStatus(def, value);
  const tone = status === "ok" ? "#2E8B57" : status === "none" ? "#9FB0AE" : "#D98324";

  /* Where the value sits across the band, clamped so an extreme reading still
     renders on the track instead of disappearing off the end. */
  const span = (def.max - def.min) || 1;
  const pos = value == null ? null
    : Math.max(2, Math.min(98, ((value - def.min) / span) * 100));
  ...
  const worst = findings && findings.length ? findings[0] : null;
  const needsDose = dose && ["suggested", "fell-short", "overshot", "due", "off-target"].includes(dose.state);

  return (
    <div className="param-row">
      <button onClick={onOpen} className="param-row-main" aria-label={`Open ${def.label} history`}>
        <span className="param-row-rule" style={{ background: def.color }} />

        <span className="param-row-label">{def.label}</span>

        <span className="param-row-value" style={{ color: value == null ? "#9FB0AE" : "#14171A" }}>
          {value == null ? "—" : fmtVal(def, value)}
          <span className="param-row-unit">{value == null ? "" : def.unit}</span>
        </span>

        {/* The band track: the target range drawn as a lit segment with the
            current value marked on it. Reading a position is faster than
            reading a number against a remembered range. */}
        <span className="param-row-track">
          <span className="param-row-band" style={{ background: def.color, opacity: 0.18 }} />
          {pos != null && (
            <span className="param-row-mark" style={{ left: `${pos}%`, background: tone }} />
          )}
        </span>

        <span className="param-row-trend" style={{ color: moved ? tone : "#C3CDD1" }}>
          {moved ? (delta > 0 ? <ArrowUp size={12} strokeWidth={3} /> : <ArrowDown size={12} strokeWidth={3} />) : "·"}
        </span>
      </button>

      <div className="param-row-tail">
        {needsDose && <span className="param-row-flag" title={dose.headline}>dose</span>}
        {worst && <span className="param-row-dot" style={{ background: worst.severity === "act" ? "#C4285B" : "#D98324" }} />}
        <button onClick={() => onLog(def.key)} className="param-row-add"
 ...
 */
function buildFindings({ readings, icps, paramDefs, settings, doseLog, waterChanges, latestByParam, kitChanges = {} }) {
  const out = [];
  ...
  for (const def of paramDefs) {
    ...
    add({
      id: "far-out-" + def.key, params: [def.key], scope: "chemistry", severity: "act",
      value: last.value,
      title: `${def.label.toLowerCase()} is a long way ${dir} range`,
      detail: `${fmtVal(def, last.value)}${def.unit} against a target of ${fmtVal(def, def.min)}–${fmtVal(def, def.max)}${def.unit}, measured ${fmtDate(last.date)}. Confirm it with a second test before making a large correction — a single reading this far out is as likely to be a test error as a real change. If it holds, correct it gradually: moving ${def.label.toLowerCase()} quickly is usually harder on livestock than the level itself.`,
    });
  }
  ...
  for (const def of paramDefs) {
    ...
    if (!(days > 2 && days <= 45)) continue;        // useful warning horizon

    add({
      id: "heading-out-" + def.key, params: [def.key], scope: "chemistry", severity: "watch",
      title: `${def.label.toLowerCase()} is heading out of range`,
      detail: `${def.label} is ${fmtVal(def, last.value)}${def.unit} and moving ${reg.slope > 0 ? "up" : "down"} at about ${fmtAmount(Math.abs(reg.slope) * 7)}${def.unit} a week. At that pace it reaches the ${reg.slope > 0 ? "top" : "bottom"} of your range in roughly ${Math.round(days)} days. Correcting a drift this size now is a small adjustment; waiting until it is outside the band means a larger one, and large corrections are harder on livestock than the drift itself.`,
    });
  }
...
export default function ReefConsole() {
  ...
  const dismissFinding = async (f) => {
    const next = { ...dismissed, [findingKey(f)]: todayStr() };
    setDismissed(next);
  ...
  const findings = useMemo(
    () => findingsData.findings.filter((f) => !dismissed[findingKey(f)]),
...
function Dashboard({ latestByParam, dueList, alerts, readings, paramDefs, saveRange, resetRange,
  ...
  return (
    <div>
      <StatusHero overview={overview} open={assessOpen}
        onOpenAssessment={() => setAssessOpen((v) => !v)} />
      ...
      <div className="param-list">
        {paramDefs.map((def) => (
          <ParamRow key={def.key} def={def} reading={latestByParam[def.key]}
