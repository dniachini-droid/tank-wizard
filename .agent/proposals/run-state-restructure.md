...
- **`.agent/backlog.md` is the next one, and it is surviving on luck.** Filed as
  ...
  **Update 2026-08-15: it did.** Three conflicts on `backlog.md` in one evening.
  Dan promoted TW-042 and the same split was applied — one file per item under
  `.agent/items/`, `backlog.md` deleted, the view generated by `npm run backlog`
  and never committed (§4 option 1 again, for the same reason). See
