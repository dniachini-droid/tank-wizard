...
SURFACES SPECIFIED 2026-08-16 — wizard-states.md §25 says what each surface
does with the verdict, which is the half §19 and §20 left as routing. What it
adds to this item: the tank summary shows one notice per parameter and its
short form is the card's headline plus its FIRST SENTENCE, generated not
written (§25.1) — so `correction-done`'s hand-written support sentence at
narrative-engine.js:474 is now a named violation of a written rule, not an
inference from §7; the summary has three layers, collapsed / expanded / a
hidden section that unhides one at a time or all at once (§25.1); HIDE and OFF
are two different things and §15 registers both — hide is per notice and
temporary, and a superseding verdict returns it to the live list automatically,
which is the behaviour `findingHidden` already implements and nothing asserts;
off is per notice type, permanent, and set in Setup. The reading confirmation
is a two-line receipt then the card verbatim (§25.3). Findings collapse to
three kinds and no fourth (§25.4). Insights stays unspecified (§25.5) — it
still receives no `doseStates`, and that is not fixed by giving it something to
render before anyone knows what it is for. Two placement questions are OPEN and
are Dan's (§25.6): app-level notices leave the summary, with Tasks the likely
but unproven home, and where relationship notices sit in a summary that is
otherwise one notice per parameter.
