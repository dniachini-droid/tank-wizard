...
claude/phosphate-nitrate-routine-7lovsr, PR #53). Both generic loops now
skip phosphate and nitrate (findings.js `NUTRIENTS_AWAITING_OWN_RULES`);
the latent noiseFloor unit misread is flagged in a comment at its read
site (helpers.js `correctionProgress`), behaviour unchanged; the repro
test exists (src/test/defects/phosphate-nitrate-borrowed-reasoning.test.js,
...
progress 2026-08-16: **the canon entry now exists** —
docs/spec/reef-chemistry.md §29, all nine of item 10's decisions answered
by Dan, item 10 closed. The reasoning half of this item is done; the
implementation half is filed as TW-054 to TW-060, untagged. Two figures
this item worked up are settled against it: the noiseFloor unit misread
is **closed as unreachable by design** (§29.8 — neither parameter has a
correction path and per §29.6 neither will), so none of the three options
recorded above is taken and the read-site comment now says so; and the
suspended husbandry expectation returns under TW-056 at a corrected
severity, because §29.4 puts nitrate above 50 at "worth attention, not
urgent" and the suspended check asserts urgency. This item stays open
until TW-054 to TW-060 ship — the notices are no longer wrong, but they
are still not yet right.
